...
import { AlertCircle, BarChart3, Briefcase } from 'lucide-react';

export default function AnalyticsPage() {
  const { analytics, isAnalyticsLoading, error: analyticsError } = useAdMetrics();
  const { projects, isLoading: projectsLoading, error: projectsError } = useProjects();

  const isLoading = isAnalyticsLoading || projectsLoading;
  const anyError = analyticsError || projectsError;

  // Safe CPC calculation — avoid divide-by-zero and misleading values
  const avgCpc =
    analytics && analytics.totalClicks > 0
      ? (analytics.totalSpend / analytics.totalClicks).toFixed(2)
      : '—';
    ...
    <Container>
      <Section className="border-b pb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Real-time ROI and Advertising Metrics across all managed accounts.
        </p>
      </Section>

      {/* Error state */}
      {anyError && <ErrorState reset={() => window.location.reload()} />}
      ...
      <Section title="Project Profitability Breakdown" className="mt-12">
        {projectsLoading ? (
          ...
          <EmptyState 
            ...
            description="Create projects to start tracking profitability and ROI metrics."
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <p className="text-slate-300 text-sm leading-relaxed">
                Based on current trends, your average agency ROAS is{' '}
                <span className="text-emerald-400 font-bold">
                  {analytics?.roas?.toFixed(2) ?? '—'}x
                </span>
                . We recommend increasing budget for projects with ROAS &gt; 4.0 to maximize total
                revenue share.
              </p>
              <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                <p className="text-xs font-bold text-slate-500 uppercase">Top Performer</p>
                <p className="text-sm mt-1">
                  Google Ads is currently driving 65% of total conversions.
                </p>
              </div>
            </div>
            <div className="space-y-4 font-mono text-xs text-slate-400">
              ...
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span>Avg. CPC:</span>
                {/* Fixed: previously used ?? 1 which showed wrong value when clicks = 0 */}
                <span className="text-white">{avgCpc === '—' ? '—' : `$${avgCpc}`}</span>
              </div>
            </div>
          </div>
        </div>
